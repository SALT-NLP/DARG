...
if os.path.exists(args.output_file):
    ...
    with open(args.original_file, "r") as file:
        for line in file:
            data = json.loads(line)
            data["Negative Attributes Filtered"] = {}
            for non_unknown_idx, _ in data["Negative Attributes"].items():
                data["Negative Attributes Filtered"][non_unknown_idx] = []
            output_data.append(data)
...
with get_openai_callback() as cb:
    for i, data in enumerate(output_data):
        for non_unknown_idx, attributes_values in data[
            "Negative Attributes Filtered"
        ].items():
            if len(attributes_values) != 0:
                print("skip")
                continue
            initial_attributes = data["Negative Attributes"][non_unknown_idx]
            ...
            if len(parsed_output) >= 3:
                data["Negative Attributes Filtered"][non_unknown_idx] = parsed_output
